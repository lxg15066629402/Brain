...
def dice(y_true, y_pred):
    ...
    dice_score = (dice_numerator + K.epsilon()) / \
        (dice_denominator + K.epsilon())
    return dice_score
...
def dice_whole_metric(y_true, y_pred):
    ...
    # y_true_f = K.concatenate([y_true_f, y_true_f], axis=0)
    # y_pred_f = K.concatenate([y_pred_f, y_pred_f], axis=0)
...
def dice_core_metric(y_true, y_pred):
    ...
    # 2018 09 14
    y_core_13 = K.stack([y_true_f[:, 1], y_true_f[:, 3]], axis=1)
    y_pred_13 = K.stack([y_pred_f[:, 1], y_pred_f[:, 3]], axis=1)
    # y_core = K.sum(y_true_f[:, 1:2:3])
    # p_core = K.sum(y_pred_f[:, 1:2:3])
    # K.stack()

    y_core = K.sum(y_core_13, axis=1)
    p_core = K.sum(y_pred_13, axis=1)
...
def gen_dice_loss(y_true, y_pred):
    ...
    # generalised dice score is calculated as in this paper :
