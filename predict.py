...
class Prediction(object):
    ...
    def predict_volume(self, filepath_image, show):
        ...
        # read nii.gz data to array
        test_im = [
            sitk.GetArrayFromImage(
                sitk.ReadImage(
                    scans_test[i])) for i in range(
                len(scans_test))]
        ...
        # reconstruct the initial target values .i.e. 0,1,2,4 for prediction
        # and ground truth
    ...
    def evaluate_segmented_volume(self, filepath_image, save, show, save_path):
        ...
        if save:
            ...
            sitk.WriteImage(tmp, 'test/{}.nii.gz'.format(save_path))
        ...
        if show:
            ...
            print(
                "Dice enhancing tumor score (jaune):{:0.4f} ".format(Dice_enhancing))
            print("**********************************************")
            ...
            print("Sensitivity core tumor score (tt sauf vert): {:0.4f}".format(
                Sensitivity_core))
            print(
                "Sensitivity enhancing tumor score (jaune):{:0.4f} ".format(Sensitivity_en))
            print("***********************************************")
            ...
            print("Specificity core tumor score (tt sauf vert): {:0.4f}".format(
                Specificity_core))
            print(
                "Specificity enhancing tumor score (jaune):{:0.4f} ".format(Specificity_en))
            print("***********************************************")
            print(
                "Hausdorff complete tumor score : {:0.4f}".format(Hausdorff_whole))
            print(
                "Hausdorff core tumor score (tt sauf vert): {:0.4f}".format(Hausdorff_core))
            print(
                "Hausdorff enhancing tumor score (jaune):{:0.4f} ".format(Hausdorff_en))
            print("***************************************************************\n\n")
    ...
    def predict_multiple_volumes(self, filepath_volumes, save, show):
        ...
        np.savetxt('test/Results.out', res)
        np.savetxt('test/Volumes_ID.out', Ids, fmt='%s')
...
if __name__ == "__main__":
    ...
    # set arguments
    # model_to_load= "Data/Models/ResUnet.05.hdf5"
    model_to_load = "model/ResUnet.05.hdf5"

    # paths for the testing data
    # path_HGG = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/BraTS-2017/Brats17TrainingData/HGG/**')
    # path_HGG = glob('Data/test_2017/HGG/**')
    # path_LGG = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/BraTS-2017/Brats17TrainingData/LGG/**')
    # path_LGG = glob('Data/test_2017/LGG/**')
    # path = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain segmentation/Brain-tumor-segmentation-master/Brain-tumor-segmentation-master/dataresult/**')
    # path = glob('Data/testdata_1/**')

    # test_path = path_HGG + path_LGG
    test_path = glob('Data/testdata_1/**')
    # test_path = glob('2018HGG60/**')

    # path_HGG = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/MICCAI_BraTS_2018_Data_Training/HGG/**')
    # path_LGG = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/MICCAI_BraTS_2018_Data_Training/LGG/**')
    # test_path = path_HGG + path_LGG

    # test_path = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/MICCAI_BraTS_2018_Data_Validation/**')
    # test_path = glob('/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/lvxiaogang/brain dataset/MICCAI_BraTS_2018_Data_Validation/**')
    ...
    # brain_seg_pred.predict_multiple_volumes(test_path[0:285],save = True, show = True)
    brain_seg_pred.predict_multiple_volumes(
        test_path[0:42], save=True, show=True)
    # brain_seg_pred.predict_multiple_volumes(test_path[0:66],save = True, show = True)