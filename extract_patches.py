...
class Pipeline(object):
    ...
    def read_scans(self, Normalize):
        ...
        for i in range(len(self.scans_train)):
            ...
            # sort() 匿名函数
            t1.sort(key=lambda x: x.split('/')[-1][0])
            ...
            tmp = [
                sitk.GetArrayFromImage(
                    sitk.ReadImage(
                        scans[k])) for k in range(
                    len(scans))]

            # crop each volume to have a size of (146,192,152) to discard some
            # unwanted background and thus save some computational power ;)
            z0 = 1
            y0 = 29
            x0 = 42
            z1 = 147
    ...
    def sample_patches_randomly(self, num_patches, d, h, w):
        ...
        # swap axes to make axis 0 represents the modality and axis 1
        # represents the slice. take the ground truth
        ...
        while (count < num_patches) and (pix > i):
            ...
            tmp = self.train_im[patient_id][0:4,
                                            slice_idx, p_y[0]:p_y[1], p_x[0]:p_x[1]]
    ...
    def _normalize(self, slice):
        ...
        if np.std(slice) == 0 or np.std(image_nonzero) == 0:
            ...
            # so that we can discard those intensities afterwards when sampling
            # random patches
            tmp[tmp == tmp.min()] = -9
            return tmp
...
def concatenate(X_patches, Y_labels):
    '''
    ...
    # data deal with
    # X_patches = X_patches[: int(len(shuffle) * 0.8)]  #
    X_patches = X_patches
    # X_patches = X_patches[: int(len(shuffle) * 0.8)]  #    # # X_patches = X_patches  #
    # X_patches_valid = X_patches[int(len(shuffle) * 0.8):]  #
    #
    # Y_labels = Y_labels[: int(len(shuffle) * 0.8)]  #
    Y_labels = Y_labels  #
    # Y_labels_valid = Y_labels[int(len(shuffle) * 0.8):]  #
    # X_patches = X_patches[: int(len(shuffle) * 0.8)]  #
    # X_patches_valid = X_patches[int(len(shuffle) * 0.8):]  #
    # Y_labels = Y_labels[: int(len(shuffle) * 0.8)]  #
    # Y_labels_valid = Y_labels[int(len(shuffle) * 0.8):]  #
...
if __name__ == '__main__':
    ...
    # path_HGG = glob('Data/train_2017/HGG/**')
    # path_LGG = glob('Data/train_2017/LGG/**')
    ...
    path_all = glob('Data/dataset_1/**')
    # path_all = path_HGG + path_LGG

    print("path_all is number", path_all)
    # shuffle the dataset
    np.random.seed(2000)
    ...
    num_patches = 146 * (end - start) * 4
    # num_patches=146 *(end-start)*2 # 146*20*3 # 9.23 6 ->10 (88 86 83)  ->20(MemoryError)  -->15  (MemoryError)  -->12 (Process finished with exit code 137 (interrupted by signal 9: SIGKILL))
    # num_patches = 170 * (end - start) * 3
    # define the size of a patch # 128 128 4
    h = 128
    w = 128
    d = 4

    # pipe = Pipeline(list_train=path_all[start:end], Normalize=True)
    pipe = Pipeline(list_train = path_all, Normalize=True)
    ...
    # this transormation is done so that we will have 4 classes when we
